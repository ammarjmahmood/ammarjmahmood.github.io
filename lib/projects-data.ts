...
The robot arm handles the fine, precise motions that Jenga demands — identifying individual blocks in the tower, extracting one without toppling the stack, and placing it back on top — all driven through the Viam platform and streamed to a remote player in real time.`,
        ...
        previewMedia: '/viam-hackathon-team.webp',
        videoUrl: 'https://youtube.com/shorts/GgyeDJ3g36w',
        detailImages: [
            '/viam-hackathon-team.webp',
            '/viam-hackathon-medal.webp',
        ],
        type: ['Mechanical', 'Software', 'Electrical'],
        tags: ['Viam', 'Hackathon', 'Robot Arm', 'Fine Motor Skills', 'Teleoperation', 'Jenga', 'Remote Control'],
        date: '2026',
        technicalStack: ['Viam Platform', 'Robot Arm', 'Computer Vision', 'Python', 'Teleoperation'],
...
To develop and test manipulation behavior without needing hardware access for every iteration, I built a CAD-accurate MuJoCo simulation of the robot: the chassis, telescopic lift, torso, and head imported directly from the Onshape assembly, and the dual arms built from their own URDF and mesh exports so joint limits and mounting geometry match the physical build. The sim runs a scripted bimanual folding task on a simulated T-shirt using MuJoCo's native cloth physics, and exports joint trajectories in the same units and rate limits the real controller expects, so a validated motion can be replayed on hardware.`,
        ...
        detailImages: [
            ...
            'https://youtube.com/shorts/J65X--nGINY',
        ],
...
The application is built with a focus on clean architecture, scalability, and ease of deployment. It demonstrates proficiency in modern Java development using the Spring Boot ecosystem.`,
            softwareArchitecture: `Backend: The backend is built with Java 17 and Spring Boot 3.2.0, utilizing Spring Web for RESTful endpoints and Spring MVC for the web interface. Data persistence is handled with an in-memory database (or configured for persistent storage) using Spring Data JPA.

Frontend: The user interface is rendered server-side using Thymeleaf templates, providing a lightweight and responsive experience without the need for a complex client-side framework.

API Design: The application exposes REST endpoints:
...
Deployment: The application is containerized using Docker, ensuring consistency across environments. It is deployed on Render, leveraging its platform-as-a-service capabilities for seamless hosting and scaling.`,
